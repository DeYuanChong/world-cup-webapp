...
- **Score sync**: `src/instrumentation.ts` starts a background poller (every `SYNC_INTERVAL_MS`, default 5 min) that calls `syncMatches()` in `src/lib/sync.ts` — one `GET /v4/competitions/WC/matches` per poll (free tier: 10 req/min). The first sync seeds all matches and teams; there is no separate seed script. `POST /api/sync` (admin-gated) triggers the same function and is the future cron target for serverless deploys where the poller doesn't run.
...
1. **`Match.homeGoals90/awayGoals90` always mean the 90-minute (+injury time) score**, never extra time/penalties. football-data.org's `score.fullTime` for extra-time matches is the 120-minute score; `extract90MinScore()` in `src/lib/football-data.ts` handles the extraction (regularTime → fullTime−extraTime fallback chain). A knockout draw is a valid result. The raw API score object is stored in `Match.rawScore` for auditing.
